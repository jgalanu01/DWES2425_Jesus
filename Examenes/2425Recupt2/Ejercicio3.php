...
$modelo = new modelo('entradas.txt');
...
<?php
    if (isset($_POST['comprar'])) {
        if (empty($_POST['nombre']) or empty($_POST['tipo']) or empty($_POST['fecha']) or empty($_POST['entradas']) ) {
            ...
            $descuentos = implode('/', $_POST['descuentos']);
            $e = new Entrada ($_POST['nombre'], $_POST["tipo"], $fecha, $_POST["entradas"],$descuentos);

            $modelo->guardarEntradas($e);
        }
    }
   
   
   
    echo '<h3> Entradas guardadas </h3>';
    echo '<table border=2px>';
    echo '<tr><th>Nombre</th><th>Tipo</th><th>Fecha</th><th>NºEntradas</th><th>Descuentos</th></tr>';
