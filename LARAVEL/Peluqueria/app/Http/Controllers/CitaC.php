...
class CitaC extends Controller
{
    function verCitas()
    {
        $citas = Cita::orderBy('fecha', 'DESC')->orderBy('hora', 'ASC')->get();
        return view('citas', compact('citas'));
    }
    function mofificarCita(Request $request, $id)
    {
    ...
    function borrarCita(Request $request, $id)
    {
        ...
        if ($cita != null) {
            if (!isset($cita->obtenerDetalle()[0])) {
                ...
                return back()->with('mensaje', 'Error, no se puede borrar la cita porque falta detalle');
            }
        } else {
            return back()->with('mensaje', 'Error la cita no existe');
        }
        ...
        if ($cita != null) {
            ...
            if ($servicio != null) {
                ...
                if ($d->save()) {
                    ...
                    return back()->with('mensaje', 'Error al añadir el servicio');
                }
            } else {
