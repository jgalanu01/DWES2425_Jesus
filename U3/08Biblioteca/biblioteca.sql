...
-- Nº de préstamos no devueltos, nº de préstamos devueltos, título del último libro prestado
...
select l.autor, count(*) from prestamos p inner join libros on p.libro = l.id
where p.socio = pIdS
group by l.autor;
